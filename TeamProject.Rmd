...
distribution.
We confirm that this is indeed the case.
As such we once again use logarithms to transform
the variable.
...
We will keep the transformation anyways and
assume normality for the rest of the project.

```{r, fig.dim = c(8, 4)}
data_trf$Age <- log(data_trf$Age)
histogram_by_groups(data_trf, "Age", "log(Age)")
```