...
Le us focus first on the mean vectors.
...
<!-- Perhaps change 0.975 to 0.99 so
it is less strict -->

We now search for outliers.
The idea is to use Mahalanobis distance.
As we suppose that our data $X ~ \mathcal{N}(\mu, \Sigma)$,
we have $D_M(x,\mu)^2 \sim \chi_p^2$, with $D_M$ the
Mahalanobis distance.
Hence we may set our outlier criteria as
\begin{equation}
    D_M(x, \mu)^2 > \chi_{p, 0.975^{1/n}}^2 \;,
\end{equation}
the $0.975^\text{th}$ quantile of the $\chi_p^2$ distribution.
