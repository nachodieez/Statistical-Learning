...
We follow with a description of our variables. 
...
Quantitative discrete variable detailing the number of times the individual has been pregnant.
...
```{r}
pred_mat <- matrix(1, ncol(data), ncol(data),
    dimnames = list("row" = colnames(data), "col" = colnames(data)))
pred_mat[, 9] <- 0
for (i in 1:8) {
  pred_mat[i, i] <- 0
}
data_im <- mice(data, m = 1, method = "pmm", predictorMatrix = pred_mat, seed = seed)
...
histogram_by_groups <- function(data, var, label = NULL) {
    ...
    p <- ggplot(data0, aes(x = eval(parse(text = var)))) +
        ...
        geom_histogram(data = data1,
            ...
            alpha = 0.65, boundary = 0, inherit.aes = FALSE) +
            theme_bw() +
            scale_fill_manual(
                name = "",
                breaks = c("Positive", "Negative"),
                values = c("Positive" = c2, "Negative" = c1)
            ) +
            xlab(label) + ylab("Relative frequency") + ggtitle(label) +
            geom_vline(xintercept = mean(data1[[var]]), colour = c2) +
            geom_vline(xintercept = mean(data0[[var]]), colour = c1)
    p + annotate(
            "text",
            x = 0.9 * max(data1[var]),
...
```{r, fig.size = c(16, 16)}
biplot(df_pcs, cex = c(0.4, 1), col = c(c2, c1))
```
...
```{r, fig.size = c(8, 4)}
fviz_eig(df_pcs, ncp = 17, addlabels = TRUE, barfill = c2, barcolor = c1)
```
...
```{r, fig.size = c(16, 16)}
pairs(df_pcs$x[, 1:4], col = vec_col, pch = 16)
```
...
abline(h = 0)
text(seq_along(xnames), m_vrm[, 1], labels = xnames, pos = 1, col = "black", cex = 0.75)
```
...
abline(h = 0)
text(seq_along(xnames), m_vrm[, 2], labels = xnames, pos = 1, col = "black", cex = 0.75)
```
...
abline(h = 0)
text(seq_along(xnames), m_vrm[, 3], labels = xnames, pos = 1, col = "black", cex = 0.75)
```
...
As we can see, the clusters are not exactly coincident with the target variable, but it does a good job. It achieves `r round((table_kmeans[1,1]+table_kmeans[2,2])/7.57, 2)`% accuracy. Let us remember that accuracy is defined as the proportion of instances correctly classified. We can also define $TER$ (test error rate) as 1 - $\text{accuracy}$.
...
```{r}
kable(cluster_classification(pam_X$cluster)) %>% kable_styling(latex_options = "HOLD_position")
```
...
plot(df_two[,1:2], col = clara_col, main = "First two PCs", 
     xlab = "First PC", ylab = "Second PC", pch = 19)
```
We can observe that the results are not so similar than with K-Means and K-Medoids. There are `r table(clara_X$cluster)[1]` instances in one group and `r table(clara_X$cluster)[2]` in the other. 
...
```{r}
kable(cluster_classification(clara_X$cluster))  %>% kable_styling(latex_options = "HOLD_position")
```
...
kable(cluster_classification(cl_single_X))  %>% kable_styling(latex_options = "HOLD_position")
```
Only one instance belongs to the second cluster.
...
kable(cluster_classification(cl_complete_X))  %>%
    kable_styling(latex_options = "HOLD_position")
```
Now, we can see that the sizes of the clusters are much more similar, although one is twice as big as the other. This is a better way of clustering our data than with single linkage.
...
kable(cluster_classification(cl_average_X))  %>%
  kable_styling(latex_options = "HOLD_position")
```
...
kable(cluster_classification(cl_ward_X))  %>%
  kable_styling(latex_options = "HOLD_position")
```

The sizes of the clusters are almost identical, and it resembles mildly the groups of the target variable (`r round((table(cl_ward_X, y2)[1,1]+table(cl_ward_X, y2)[2,2]) / 757, 2)`% accuracy). 
...
```{r}
colors_db <- c("green", "red", "orange", "blue")[Mclust_X$classification]
plot(df_two[,1:2], pch = 19, col = colors_db, xlab = "First PC",
     ylab = "Second PC")
```
There is significant overlap between the clusters, and it is curious that the separation is not vertical. This suggests that the way of clustering the data is not doing so by the groups created by the target variable.
...
If we were to choose only one technique, we would go with K-Means or K-Medoids as we know beforehand that there are two groups in the data, which makes partitional clustering a good choice. 
...
```{r, echo=FALSE}
table <- data.frame("Instances_actually_negative" = c("True negative (TN)", "False positive (FP)"),
                    "Instances_actually_positive" = c("False negative (FN)", "True positive (TP)"))
rownames(table) <- c("Classified_as_negative", "Classified_as_positive")

kable(table) %>% kable_styling(latex_options = "striped") %>%
    kable_styling(latex_options = "HOLD_position")
```
With this in mind, we can also define some important metrics for inbalanced problems:

- $\text{Sensitivity}=TP/(TP+FN)$ | (True Positive Rate)
- $\text{Specificity}=TN/(FP+TN)$ | (True Negative Rate)
- $\text{Accuracy}=(TP+TN)/(TP+TN+FP+FN)$
- $\text{TER} = (FP+FN)/(TP+TN+FP+FN) = 1 - \text{Accuracy}$ | (Test Error Rate)
...
It seems that the optimal $K$ is `r k`.
...
And, with the predictions computed, we can show the confusion matrix:
```{r}
cm_knn <- confusionMatrix(table(knn_Y_test,Y_test), positive = "Positive")
...
```{r}
bac <- function(cm){
  return((cm[1,1]/(cm[1,1] + cm[2,1]) + (cm[2,2]/(cm[1,2] + cm[2,2])))/2)
}
```
...
```{r}
prob_knn_Y_test <- ifelse(knn_Y_test == "Positive",
    attributes(knn_Y_test)$prob, 1 - attributes(knn_Y_test)$prob)
prob <- 2*ifelse(knn_Y_test == "-1", prob_knn_Y_test, 1-prob_knn_Y_test) - 1
colors_errors <- c("indianred2", "seagreen2")[1*(Y_test==knn_Y_test)+1]
...
Next, we are going to test is logistic regression. We start by training the model and checking its summary:
```{r}
...
```{r}
cm_lg_default <- confusionMatrix(table(lr_test,Y_test), positive = "Positive")
cm_lg_default$table %>% kable() %>% kable_styling(latex_options = "striped") %>%
    kable_styling(latex_options = "HOLD_position")
```
It can be observed that the model classifies well those individuals who do not have diabetes, but has a hard time classifying those individuals who have diabetes. This probably is because the problem is inbalanced.
...
colors_errors <- c("indianred2", "seagreen2")[1*(Y_test==lr_test)+1]
...
In order to do so, we are going to first do stepwise model selection. 
The information criterion that we are going to use in order to evaluate the measure the performance of the model is the *Bayesian Information Criterion* (BIC),
...
cm_lr <- confusionMatrix(table(pred,Y_test), positive = "Positive")
...
cm_lr_opt <- confusionMatrix(table(pred,Y_test), positive="Positive")
cm_lr_opt$table %>% kable() %>% kable_styling(latex_options = "striped") %>%
    kable_styling(latex_options = "HOLD_position")
```
```{r, echo=FALSE}
predictors_tables[[3]] <- cm_lr_opt
```
...
```{r}
colors_errors <- c("indianred2", "seagreen2")[1*(Y_test==pred)+1]
plot(1:n_test,prob_lr_test,col=colors_errors,pch=20,type="p",
...
```{r}
cm_LDA <- confusionMatrix(table(lda_Y_test,Y_test), positive="Positive")
cm_LDA$table %>% kable() %>% kable_styling(latex_options = "striped") %>%
    kable_styling(latex_options = "HOLD_position")
```

```{r, echo=FALSE}
predictors_tables[[4]] <- cm_LDA
```
...
lda_accuracy
```
If we compute the BAC we can see that it is about the same as the default logistic regression.

```{r}
bac(cm_LDA$table)
```

Despite its bayesian nature, the sample size is probably too small for this to really have a real impact when taking into account the unbalanced problem.
...
pred <- pred * 1
pred <- as.factor(c("Negative", "Positive")[(1*pred)+1])
cm_lda_opt <- confusionMatrix(table(pred,Y_test), positive="Positive")
cm_lda_opt$table %>% kable() %>%
  kable_styling(latex_options = "striped") %>% kable_styling(latex_options = "HOLD_position")
```

```{r, echo=FALSE}
predictors_tables[[5]] <- cm_lda_opt
```
...
```{r}
cm_QDA <- confusionMatrix(table(qda_Y_test,Y_test), positive="Positive")
cm_QDA$table %>% kable() %>%
    kable_styling(latex_options = "striped") %>% kable_styling(latex_options = "HOLD_position")
```
...
colors_errors <- c("indianred2", "seagreen2")[1*(Y_test==qda_Y_test)+1]
plot(1:n_test,probs,col=colors_errors,pch=20,type="p",
     xlab="Test sample",ylab="Probabilities of Diabetes",
     main="Probabilities of Diabetes")
abline(h=0.5)
...
```{r}
test_prob = predict(qda_train, newdata = X_test, type = "response")
test_prob <- test_prob$posterior[,2]

ROC_qda = plot.roc(Y_test, test_prob,
               main="Confidence interval of a threshold", percent=TRUE,
               ci=TRUE, of="thresholds", 
               thresholds="best", 
               print.thres="best",
...
According to the ROC curve, the optimum threshold is about `r as.numeric(rownames(ROC_qda$ci$specificity))`.

If we compute the predictions with the new threshold:

```{r}
pred <- predict(qda_train, X_test, type = "response")$posterior[,2] > 
  as.numeric(rownames(ROC_lda$ci$specificity))
pred <- pred * 1
pred <- as.factor(c("Negative", "Positive")[(1*pred)+1])
cm_qda_opt <- confusionMatrix(table(pred,Y_test), positive="Positive")
cm_qda_opt$table %>% kable() %>%
    kable_styling(latex_options = "striped") %>% kable_styling(latex_options = "HOLD_position")
```

We can see a better confusion matrix.

```{r, echo=FALSE}
predictors_tables[[7]] <- cm_qda_opt
```
...
```{r}
cm_nb <- confusionMatrix(table(nb_Y_test,Y_test), positive = "Positive")
cm_nb$table
```
```{r, echo=FALSE}
predictors_tables[[8]] <- cm_nb
```
...
The results are good but nothing unbeliable. We can also compute the BAC:

```{r}
bac(cm_nb$table)
```
The BAC is similar to the accuracy.
...
colors_errors <- c("indianred2", "seagreen2")[1*(Y_test==nb_Y_test)+1]
...
According to the ROC curve, the optimum threshold is `r as.numeric(rownames(ROC_nb$ci$specificity))`


We compute the predictions with the new threshold:

```{r}
pred <- predict(nb_train, as.matrix(X_test), type = "prob")[,2] > 
  as.numeric(rownames(ROC_lda$ci$specificity))
pred <- pred * 1
pred <- as.factor(c("Negative", "Positive")[(1*pred)+1])
cm_nb_opt <- confusionMatrix(table(pred,Y_test), positive="Positive")
cm_nb_opt$table %>% kable() %>%
    kable_styling(latex_options = "striped") %>% kable_styling(latex_options = "HOLD_position")
```


```{r, echo=FALSE}
predictors_tables[[9]] <- cm_nb_opt
```
...
```{r}
BAC <- bac(cm_nb_opt$table)
BAC
```

It is lower than the BAC computed for the logistic regression after tuning the threshold.

Finally, we plot the probability of being positive of diabetes and the threshold:
```{r}
probs <- predict(nb_train, as.matrix(X_test), type = "prob")[,2]
colors_errors <- c("indianred2", "seagreen2")[1*(Y_test==pred)+1]
plot(1:n_test,probs,col=colors_errors,pch=20,type="p",
     xlab="Test sample",ylab="Probabilities of Diabetes",
     main="Probabilities of Diabetes")
...
for(i in 1:length(predictors_tables)){
  v <- c(round(predictors_tables[[i]]$overall["Accuracy"],4),
         round(bac(predictors_tables[[i]]$table),4),
         round(predictors_tables[[i]]$byClass["Sensitivity"],4),
         round(predictors_tables[[i]]$byClass["Specificity"],4)
         )
  results_table <- cbind(results_table, v)
}
results_table <- results_table[,-1]
rownames(results_table) <- c("Accuracy", "BAC", "Sensitivity", "Specificity")
colnames(results_table) <- c("KNN", "LR","LR_roc",
                            "LDA","LDA_roc", "QDA","QDA_roc", "NB", "NB_roc")
...
On the other hand, and as we have commented throughout this section, Bayesian methods have the advantage in this type of problem that they take into account the prior probability, so that by default they already consider that the problem is unbalanced. However, and perhaps due to the small sample size, they have not been particularly superior to the rest by default.

Finally, we believe that considering what levels of sensitivity and specificity we want to obtain is especially important in medical problems (such as the one we are facing), since, for certain problems, we may be especially interested in correctly classifying positive individuals (Sensitivity), in others negative individuals (Specificity), or in others in finding a balance between them (Balanced Accuracy).

# Conclusions

In this project, we have taken a dataset comprised of medical
information about testings done in patients and whether they have or
not diabetes.
We took on the task of building a model which could
successfully predict based on the clinical analysis the outcome
of a diabetes test on a person.
For this, we used a huge variety of statistical techniques.

First, with the help of multivariate analysis and
dimensionality reduction methods, we grasped on the important
features seggregating our two population groups.
We also learned about the particularities of the values
taken by the medical results of these patients.
In the process, cleaning and correct treatment of the data
was enforced, applying the necessary preprocessing steps,
like imputing or discarding outliers, whenever it was necessary.

Later, an unsupervised classification approach was taken.
We resorted to many clustering methods to group all the predictors.
The idea was to try and find if, by just looking at the features
we may get an insight of the differences between the positive
and negative population.
In the process, we compared the different methos, arguing
when some of them were not appropiate for our task.

At last, a supervised classification was developed,
taking a look at many of the avaible methods and with
an special focus on the inbalance of this problem.
We made use of advanced techniques like ROC curve
which allowed us to improve the model and give a versatility
depending of the targeted objective.

All together, we completed the assigned task
with success.